#!/usr/bin/env python3
"""
AlexNet Training with Model Saving After Each Fold
Prevents data loss during long training sessions
"""
...
def main():
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(train_images, train_labels), 1):
        ...
        for epoch in range(1, config.num_epochs + 1):
            ...
            if val_acc > best_val_acc:
                ...
                # Save best model for this fold during training
                torch.save(model.state_dict(), f'best_model_fold_{fold}_temp.pth')
        
        print(f"\n  Fold {fold} Best Val Accuracy: {best_val_acc:.4f} ({best_val_acc*100:.2f}%)")
        
        # *** SAVE MODEL AFTER EACH FOLD ***
        fold_model_path = f'model_fold_{fold}.pth'
        torch.save({
            'fold': fold,
        ...
        with open('cv_results_intermediate.json', 'w') as f:
            json.dump({
                'completed_folds': fold,
                'fold_results': cv_results,
                'mean_so_far': float(np.mean([r['best_val_acc'] for r in cv_results])),
            }, f, indent=2)
        print(f"  ✓ Progress saved: cv_results_intermediate.json\n")
    ...
    print(f"✓ All fold models saved: model_fold_1.pth through model_fold_{config.num_folds}.pth")
    print(f"{'='*80}\n")
