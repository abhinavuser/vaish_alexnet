...
def analyze_dataset():
    """Analyze the tomato leaves dataset comprehensively"""
    ...
    # Analyze label content (H/D in filename indicates healthy/diseased)
    def count_labels(label_path, label_dir_name):
        """Count healthy and diseased labels in a directory"""
        healthy_count = 0
        diseased_count = 0
        ...
        for label_file in (label_path / label_dir_name).iterdir():
            if label_file.is_file() and label_file.suffix == '.txt':
                ...
                # Check the filename for H or D prefix
    ...
    results['data_quality']['missing_labels_training_files'] = missing_labels_train[:10]  # First 10
    results['data_quality']['missing_labels_validation_files'] = missing_labels_val[:10]  # First 10
